...
const HomeMenuItem = ({ code, name, isLocked, onClick }: Props) => {
    const isLockedCx = isLocked ? 'grayscale' : '';

    // I think it's better if there's alert if the menu item is locked
    return (
        <div
            className={`flex flex-col justify-center items-center bg-white py-8 rounded-2xl shadow-sm shadow-blue-800/20 cursor-pointer ${isLockedCx}`}
